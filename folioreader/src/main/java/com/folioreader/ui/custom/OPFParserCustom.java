...
public class OPFParserCustom {
    ...
    public static EpubPublicationCustom parseOpfFile(String rootFile, EpubPublicationCustom publication, Container container) throws EpubParserException {
    ...
    private static void parseSpineAndResourcesAndGuide(Document document, EpubPublicationCustom publication, String coverId, String rootFile, Container container) throws EpubParserException {
        int startIndex = 0;
        ...
        Map<String, CustomLink> manifestLinks = new HashMap<>();
        ...
        if (itemNodes != null) {
            for (int i = 0; i < itemNodes.getLength(); i++) {
                ...
                if (hrefFromItem.contains("ncx")) {
                    NcxParserCustom.parseNCXFile(link.getHref(), container, publication, packageName);
                }
        ...
        if (itemRefNodes != null) {
            for (int i = 0; i < itemRefNodes.getLength(); i++) {
                ...
                String linear = itemRefElement.getAttribute("linear");
                if (manifestLinks.containsKey(id)) {
                    CustomLink link = manifestLinks.get(id);
                    link.linear = linear;
